...
WORKDIR /go/src/github.com/appxecute/score-ecs
...
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-X github.com/appxecute/score-ecs/internal/version.Version=${VERSION}" -o /usr/local/bin/score-ecs ./cmd/score-ecs
...
COPY --from=builder /usr/local/bin/score-ecs /usr/local/bin/score-ecs

# Run the binary.
ENTRYPOINT ["/usr/local/bin/score-ecs"]
